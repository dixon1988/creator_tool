...
function readDir (dir, obj) {
    ...
    for (var i = 0; i < subpaths.length; ++i) {
        ...
        if (stat.isDirectory()) {
            ...
            // let tempBuff = fs.readFileSync(subpath, 'binary');
            // md5 = crypto.createHash('md5').update(tempBuff).digest('hex');
            
            compressed = path.extname(subpath).toLowerCase() === '.zip';
            
            // let manifested = path.extname(subpath).toLowerCase() === '.manifest';
            // if (manifested) {
            //     console.log(subpath + ' is igonrefile');
            //     continue;
            // }

            let tempBuff = fs.readFileSync(subpath);
            md5 = crypto.createHash('md5').update(tempBuff).digest('hex');
