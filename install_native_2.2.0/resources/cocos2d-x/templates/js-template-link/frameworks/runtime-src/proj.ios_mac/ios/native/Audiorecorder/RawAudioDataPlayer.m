...
@synthesize mPcmData;
@synthesize emptyAudioQueueBufferIndexs;

- (id)init {
    self = [super init];
    if (self)
    {
        self.mPcmData = [NSMutableData data];
        readedBytes = 0;
        pcmDataBuffer = malloc(EVERY_READ_LENGTH);
        synlock = [[NSLock alloc] init];
        self.emptyAudioQueueBufferIndexs = [NSMutableArray arrayWithCapacity:QUEUE_BUFFER_SIZE];
    }
    return self;
}
...
{
    self.mPcmData=nil;
    self.emptyAudioQueueBufferIndexs=nil;
    free(pcmDataBuffer);
    [super dealloc];
}
