...
- (id)initWithFileName:(NSString*)filename
{
    if (self = [super init])
    {
        mFileName = [[NSString stringWithString:filename] retain];
        ...
        bufferData = [[NSMutableData data] retain];
        tempData = [[NSMutableData data] retain];
        pcmDatas = [[NSMutableArray array] retain];
    ...
    @synchronized(pcmDatas)
    {
        ...
        while ([tempData length] >= packetSize)
        {
            @autoreleasepool
            {
                ...
                NSMutableData* pOldData = tempData;
                tempData = [[NSMutableData dataWithBytesNoCopy:dataPtr length:[tempData length] - packetSize freeWhenDone:NO] retain];
                [pOldData release];
                pOldData=nil;
            }
        }
...
- (void)main
{
    SpeexCodec *codec = [[[SpeexCodec alloc] init] autorelease];
    [codec open:4];     //压缩率为4
